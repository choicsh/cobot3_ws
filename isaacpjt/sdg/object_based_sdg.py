...
for obj in labeled_assets_and_properties:
    ...
    for i in range(count):
        ...
        # Local files are used as-is, everything else is resolved against the nucleus assets root
        if obj_url.startswith(("omniverse://", "file://")) or os.path.isfile(obj_url):
            asset_path = obj_url
        else:
            asset_path = assets_root_path + obj_url
        prim.GetReferences().AddReference(asset_path)
        # Drop any rigid body coming from the asset itself, the wrapper prim below becomes the rigid body
        # (nested rigid bodies are not supported by PhysX)
        for desc_prim in Usd.PrimRange(prim):
            if desc_prim != prim and desc_prim.HasAPI(UsdPhysics.RigidBodyAPI):
                desc_prim.RemoveAPI(UsdPhysics.RigidBodyAPI)
...
for i in range(num_cameras):
    ...
    for key, value in camera_properties_kwargs.items():
        if cam_prim.HasAttribute(key):
            # yaml gives lists, Gf vector attributes (e.g. clippingRange) only accept tuples
            cam_prim.GetAttribute(key).Set(tuple(value) if isinstance(value, list) else value)
        else:
...
# Spin the labeled assets in place (angular only, linear velocity is left to the 'pull to center' logic
# so the assets stay close to where the cameras were aimed)
labeled_angular_velocity_max = config.get("labeled_angular_velocity_max", 180)


def randomize_labeled_angular_velocities(assets):
    for prim in assets:
        prim.GetAttribute("physics:angularVelocity").Set(
            tuple(random.uniform(-labeled_angular_velocity_max, labeled_angular_velocity_max) for _ in range(3))
        )
...
for i in range(num_frames):
    ...
    # Keep the floating labeled assets tumbling so the dataset covers all orientations.
    # Gravity is off for them, so nothing else would ever change their spawn rotation.
    if floating_labeled_prims:
        randomize_labeled_angular_velocities(floating_labeled_prims)
