...
"""
# 단순하게 파이썬에서 각 동작에 대한 이터레이터를 구축하는 것으로 보면 됨.
# 여기서는 로봇의 단계별 이동/그리퍼 동작 들을 순차적으로 구현한 것.
# 물론 디테일하게 비교하면 호출 할 때 마다 다음 단계 실행(이터레이터)과
# 내부 조건에 따라 자동적으로 다음 단계 실행(FSM)이겠지만...
