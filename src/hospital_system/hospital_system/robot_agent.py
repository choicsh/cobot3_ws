...
class RobotAgent(Node):
    def __init__(self):
        ...
        self.declare_parameter("pose_topic", "/amcl_pose")  # Nav2 네임스페이스는 P7
        ...
        if self.get_parameter("use_db").value:
            ...
            self.create_timer(1.0, self.rec.heartbeat)
            self.create_subscription(
                PoseWithCovarianceStamped, self.get_parameter("pose_topic").value, self._on_pose,
    ...
    def _on_pose(self, msg):
        ...
        self.rec.pose(p.x, p.y, math.atan2(2 * (q.w * q.z + q.x * q.y), 1 - 2 * (q.y * q.y + q.z * q.z)))
