...
from nav_to_goal.image_saver_node import capture_single_frame
...
def main():
    ...
    waypoints.append(create_pose(nav, 2.6, 0.5, 0))   # 경유지 1
    waypoints.append(create_pose(nav, -3.9, 11.6, 135))  # 경유지 2
    waypoints.append(create_pose(nav, -5.7, 0.18, 270))  # 경유지 3 (최종 목적지)
    ...
    while True:
        for wp in waypoints:
            ...
            print(wp)
            is_success = capture_single_frame(
                topic_name='/front_stereo_camera/left/image_raw',
                save_path=f'~/captured/left_frame_{waypoints.index(wp)}.png',
                timeout_sec=5.0
                )
            if is_success:
                print("캡처 완료! 다음 작업을 진행합니다.")
            else:
                print("캡처 실패 또는 토픽 응답 없음.")
