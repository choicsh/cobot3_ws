...
def main():
    ...
    #                       planner 에 맡기지 않는다.
    #   [B] 자율주행      : NavigateThroughPoses 로 경유점(복도 입구 -> 복도 출구)을 거쳐 도킹 라인 앞 staging 지점까지.
    #                       planner(Smac Hybrid-A*, 회전 반경 0.6) 가 1 Hz 로 재계획하므로 사람이 막으면 우회하고,
    #                       경유점 + 차선 마스크 덕에 평소엔 ㄷ자 동선 가운데로 간다. 경유점은 반경 0.7 m 안에 들어오면
    #                       통과한 것으로 치고 멈추지 않는다 (RemovePassedGoals).
    ...
    # [B] 경유점 (x, y, yaw_deg). 마지막이 staging 지점 = 도킹 라인 위, 책상 구간(x<=3.4) 에서 2.1 m 앞.
    via_poses = [
        (7.0, 5.2, 90.0),      # 복도 입구 (아래 방 위쪽 경계, 복도 중앙 x=7.0) 북향
        (7.0, 12.1, 90.0),     # 복도 출구 (위 방 아래쪽 경계) 북향
        (5.5, 19.163, 180.0),  # staging: 도킹 라인 위, 서향
    ]
    dock_route = [
    ...
    bt_xml = os.path.join(get_package_share_directory('carter_navigation'),
                          'behavior_trees', 'navigate_through_poses_carter.xml')   # Spin/BackUp 복구 제거판
    ...
    def run_through_poses(name, poses):
        print(f"[{name}] NavigateThroughPoses -> {' -> '.join(str(p) for p in poses)}")
        nav.goThroughPoses([create_pose(nav, *p) for p in poses], behavior_tree=bt_xml)
        last_print = 0.0
        while not nav.isTaskComplete():
            ...
            if feedback and now - last_print > 3.0:
                last_print = now
                print(f"-> 남은 거리 {feedback.distance_remaining:.1f} m, 남은 경유점 {feedback.number_of_poses_remaining}개, "
    ...
    stages = [
        ...
        ('B 자율주행', lambda: run_through_poses('B 자율주행', via_poses)),
