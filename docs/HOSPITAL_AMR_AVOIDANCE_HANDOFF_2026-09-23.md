...
- Planner 서버가 아직 준비되지 않았다면 `getPath`의 무기한 서버 대기에 들어가지 않고 이번 요청을 건너뛴다. Planner/Smoother의 **결과 응답 자체**는 Nav2 Simple Commander의 동기 호출을 쓰므로 아직 별도 기한이 없다. 이 구간에서 응답이 늦는 경우 guard와 Collision Monitor가 명령 신선도에 따라 정지시키지만 mission은 응답을 기다린다.
