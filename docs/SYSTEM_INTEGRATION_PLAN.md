...
**도킹 실패 보완 (2026-09-26)** — `scripts/run_system.sh` 시험(1대)에서 채취실 도킹이 3번 모두 실패: 정류장에서 AMCL 이 옆으로 0.15 m
틀린 채(라이다 간격 0.00 m) 직진 시작 → 조향으로 간격을 고치려 틀자 뒤 차체가 책상 쪽으로 돌아 모서리 간격 −0.19 m 로 중단 → 두 번째 시간
초과 → 세 번째는 도킹 직선 안(도킹 0.4 m 전, 책상 쪽 0.22 m)에서 이어 가기가 "경로 위 아님" 으로 출발 거부.
- `TableDocking.move`: 정렬 뒤 정류장에서 라이다로 책상 간격을 재고 목표(0.15 m)에서 6 cm 넘게 벗어나면 직진하지 않는다(`misaligned`).
  실패 이유를 `failure` 에 남긴다. 제한 시간 90 → 150 s (책상에 붙은 쪽에서 충돌 감시가 속도를 줄여 1.2 m 에 85 s 걸린 정상 접근이 끊겼다).
- `back_out`: 지금 방향 그대로 곧게 후진해 도킹 자세 4.0 m 전(정류장 1.5 m 뒤, 도착 경로 마지막 직선 위)까지. 책상 옆에서는 돌지 않는다 —
  처음엔 방향을 고치며 후진했는데 뒤 차체가 책상으로 돌아 충돌 감시가 멈춰 세웠다.
- `hospital_mission.dock_with_retries`: 실패하면 책상 옆에서 라이다로 AMCL 재측위(옆 오차를 고친다) → 곧게 후진 → 도착 경로 마지막 직선을
  다시 달려 정류장 → 도킹, 2번까지. 물러난 자리에서는 재측위하지 않는다(가장자리 끝이 검출 범위 4 m 밖이라 앞뒤로 0.8 m 틀렸다).
  이어 가기가 도킹 직선 안에서 시작되면(`on_dock_line`) 물러나기부터.
- 실기(로봇 1대, 보행자·팔 끔): A 도킹 직선 안(책상 쪽 0.17 m, 1.7도)에서 이어 가기 → 곧게 4.85 m 후진, 재접근, 첫 도킹 시간 초과(90 s 시절)
  → 두 번째에 도킹(간격 0.150, 앞뒤 1.5 cm). B 정류장에서 책상 쪽 0.12 m → `misaligned`(간격 0.029) → 한 번 물러나 재접근 → 22 s 만에 도킹
  (간격 0.149, 앞뒤 3 mm). CPU 시험: 물러난 자리가 두 책상 모두 도착 직선 위, 도킹 도중 멈춘 자세 판정.

**복제 로봇 투명 + 정적 장애물 앞 정지 (2026-09-26)** — 사용자 3대 실행(18:33)에서 발견.
- 투명: 로봇 모델의 차체 메시는 인스턴싱(공유 프로토타입)이다. 씬을 연 뒤 복제하면 프로토타입이 다시 배정되는데 Fabric 렌더 델리게이트가
  못 따라가 한 대(robot_nova_3)의 메시 31개가 `cannot find protoPath` 로 안 그려졌다(팔은 보이고 차체·바퀴만 투명). `scene.add_robots` 가
  복제 로봇의 instanceable 프림을 세션 레이어에서 끈다(복사와 한 ChangeBlock). 재현 실행: 오류 0건, 3대 모두 차체가 보임, GPU 4.8/8 GB 그대로.
- 정지: robot1 이 upper_reserve (-7.06, 17.02) 에서 정적 막힘 바로 앞에 섰다. 옆 후보(S자 3~5 m)도 NavFn 우회(급회전, geometry 탈락)도
  없어 양보 예산 15 s → 실패 → 같은 자리 재개 → 또 실패를 되풀이. `follow_stage` 에 탈출 추가: 정적 막힘이 5 m 안에서 4 s 넘게 이어지면
  방금 지나온 차선을 따라 곧게 1.5 m(안 되면 1.0 m) 후진(`cmd_vel_nav`, 속도 0.12 m/s, 출력 가드·충돌 감시 경유) 뒤 옆 후보·NavFn 을
  곧바로 다시 보고 양보 예산도 새로. 꼬리 1.38 m 는 라이다 사각이라 차선 방향 20도 이내, 비용 지도 비어 있음, 추적 중인 사람과 1 m 이상일 때만,
  후진 중 사람이 0.4 m 안이면 멈춘다. 구간마다 2번까지. 재개는 구간이 로봇 자리에서 시작하므로 구간 시작점은 제한하지 않는다.
  CPU 시험 2개(두 번 물러나 다시 평가, 뒤에 사람이 있으면 후진 안 함). Isaac 실기 확인은 아직.
